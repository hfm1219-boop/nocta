...
import { comprarEntradas, TIPOS_ENTRADA } from "@/lib/tickets";
  ...
  return (
    <main className="flex-1 px-5 py-8 max-w-lg mx-auto w-full space-y-6">
      ...
      <section className="space-y-3">
        {tipos.map((item) => (
          <button key={item.id} onClick={() => setTipoId(item.id)} className={`card w-full p-4 text-left flex justify-between gap-4 ${item.id === tipoId ? "chip-active" : ""}`}>
            <span><b className="block">{item.nombre}</b><span className="text-xs text-muted">{item.descripcion}</span></span>
            <b className="shrink-0">{item.precio ? `$${item.precio.toLocaleString("es-CO")}` : "Gratis"}</b>
          </button>
        ))}
      </section>
      ...
      <button onClick={confirmar} disabled={!titular.trim() || !email.includes("@") || procesando} className="btn-neon w-full rounded-2xl p-4 font-bold disabled:opacity-40">
