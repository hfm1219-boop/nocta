...
export default function MisReservas() {
  ...
  return <main className="flex-1 px-5 py-8 max-w-lg mx-auto space-y-6">
    <header className="flex justify-between gap-3"><Logo size="text-3xl" /><div className="flex items-center gap-3"><Link href="/">← Explorar</Link><CerrarSesion /></div></header>
    <nav className="grid grid-cols-2 gap-2"><Link href="/mis-entradas" className="card p-3 text-center">Entradas</Link><span className="chip-active rounded-xl border p-3 text-center">Reservas</span></nav>
    ...
    {!reservas.length && <div className="card p-10 text-center text-muted">Todavía no tienes reservas.</div>}
  </main>;
}
