...
type Reward = { slug: string; name: string; description: string; image_url?: string; category?: string; points_required: number; stock: number; terms: string; venues?: { name?: string } | null };
...
export default function MiNocta() {
  ...
  return <main className="flex-1 px-5 py-8 max-w-6xl mx-auto w-full space-y-8">
    ...
    <section id="beneficios"><Heading title="Beneficios" detail="Reserva con puntos y úsalo en el comercio antes del vencimiento."/><div className="grid md:grid-cols-3 gap-4 mt-4">{data.rewards.map((reward) => <article key={reward.slug} className="card p-5 flex flex-col overflow-hidden relative"><div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-neon1/10 blur-2xl"/><div className="relative"><span className="text-3xl">{reward.image_url||"🎁"}</span><p className="text-xs uppercase tracking-wider text-neon3 mt-3">{reward.venues?.name} · {reward.stock} disponibles</p><h3 className="text-xl font-bold mt-2">{reward.name}</h3><p className="text-sm text-muted mt-2 min-h-12">{reward.description}</p>{reward.terms&&<p className="text-xs text-muted mt-3">{reward.terms}</p>}<p className="font-bold text-lg mt-4">{reward.points_required.toLocaleString("es-CO")} puntos</p><button onClick={()=>void act({action:"redeem",rewardSlug:reward.slug},reward.slug,"Beneficio reservado. Ya puedes mostrar el QR.")} disabled={reward.stock<1||data.balance.available<reward.points_required||Boolean(busy)} className="btn-neon rounded-xl p-3 mt-3 w-full disabled:opacity-40">{busy===reward.slug?"Reservando…":reward.stock<1?"Agotado":data.balance.available<reward.points_required?`Te faltan ${reward.points_required-data.balance.available} puntos`:"Canjear beneficio"}</button></div></article>)}{!data.rewards.length&&<div className="card p-8 text-center md:col-span-3"><p className="text-4xl">🎁</p><h3 className="text-xl font-bold mt-3">Nuevos beneficios en camino</h3><p className="text-sm text-muted mt-2">Mientras se publican, completa misiones para acumular puntos.</p></div>}</div></section>
