...
type Catalog = {
  ...
  promotions: Array<{ id: string; title: string; activation_id?: string; promotion_rules?: Array<{ id: string; mechanic: string }> }>;
};
...
export function PromotionEngineAdmin({ venueId }: { venueId: string }) {
  ...
  async function send(body: Record<string, unknown>, form: HTMLFormElement) {
    ...
    form.reset(); setNotice("Configuración guardada y disponible para evaluación."); await load();
  }
  ...
  return <section className="space-y-4 border-t border-line pt-6">
    ...
    {data && <div className="grid lg:grid-cols-2 gap-4">
      ...
      <form onSubmit={createRule} className="card p-5 space-y-3"><h3 className="font-bold">2. Regla y atribución</h3><Select name="promotionId" label="Promoción" items={data.promotions.map(x => ({ id: x.id, name: `${x.title}${x.promotion_rules?.length ? " · configurada" : ""}` }))}/><Select name="activationId" label="Activación opcional" optional items={data.activations.map(x => ({ id: x.id, name: `${x.brandName} · ${x.campaignName} · ${x.name}` }))}/><Select name="menuItemId" label="Producto beneficiado" items={data.menuItems.map(x => ({ id: x.id, name: x.name }))}/><Select name="brandProductId" label="SKU atribuible (obligatorio con activación)" optional items={data.brandProducts.map(x => ({ id: x.id, name: `${x.brandName} · ${x.sku}` }))}/><select name="mechanic" className="entrada bg-background"><option value="percentage">Porcentaje de descuento</option><option value="fixed_amount">Monto fijo</option><option value="buy_x_get_y">Compra X y recibe Y</option><option value="fixed_price">Precio fijo</option></select><div className="grid grid-cols-3 gap-2"><input name="benefit" type="number" min="1" className="entrada" placeholder="% / valor"/><input name="buyQuantity" type="number" min="1" className="entrada" placeholder="Compra X"/><input name="getQuantity" type="number" min="1" className="entrada" placeholder="Recibe Y"/></div><div className="grid grid-cols-2 gap-2"><input name="minimumQuantity" type="number" min="1" defaultValue="1" className="entrada" placeholder="Cantidad mínima"/><input name="minimumSpendCop" type="number" min="0" className="entrada" placeholder="Compra mínima COP"/></div><div className="grid grid-cols-2 gap-2"><input name="maximumDiscountCop" type="number" min="1" className="entrada" placeholder="Descuento máximo"/><input name="perUserLimit" type="number" min="1" className="entrada" placeholder="Límite/usuario"/><input name="totalLimit" type="number" min="1" className="entrada" placeholder="Cupo total"/><input name="budgetCop" type="number" min="1" className="entrada" placeholder="Presupuesto"/></div><div className="grid grid-cols-2 gap-2"><label className="text-xs text-muted">Desde<input name="timeStart" type="time" className="entrada mt-1"/></label><label className="text-xs text-muted">Hasta<input name="timeEnd" type="time" className="entrada mt-1"/></label></div><p className="text-xs text-muted">Al vincular una activación, NOCTA exige un mapping SKU ↔ menú aprobado por ambas partes y usa únicamente ventas pagadas y entregadas.</p><button disabled={busy} className="btn-neon rounded-xl p-3 w-full disabled:opacity-50">Activar regla</button></form>
    </div>}
  </section>;
}

function Select({ name, label, items, optional = false }: { name: string; label: string; items: Array<{ id: string; name: string }>; optional?: boolean }) {
  return <label className="block text-xs text-muted">{label}<select name={name} required={!optional} className="entrada bg-background mt-1"><option value="">{optional ? "Sin vincular" : "Seleccionar"}</option>{items.map(x => <option key={x.id} value={x.id}>{x.name}</option>)}</select></label>;
}
