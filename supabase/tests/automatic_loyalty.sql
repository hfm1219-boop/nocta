...
begin
  ...
  insert into public.consumer_events(event_type,user_id,session_id,venue_id,source,dedup_key)
  values('venue_checkin',test_user_id,gen_random_uuid(),test_venue_id,'test','__phase5_checkin_1__'||gen_random_uuid())returning id into first_event;
  checkin_key:='automatic:venue-checkin:'||test_user_id||':'||test_venue_id||':'||to_char(now() at time zone 'America/Bogota','YYYY-MM-DD');
  ...
  insert into public.consumer_events(event_type,user_id,session_id,venue_id,source,dedup_key)
  values('venue_checkin',test_user_id,gen_random_uuid(),test_venue_id,'test','__phase5_checkin_2__'||gen_random_uuid());
