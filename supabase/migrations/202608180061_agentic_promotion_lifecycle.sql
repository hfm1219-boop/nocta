-- Extiende el agente para administrar el ciclo de vida de promociones con confirmaciones de un solo uso.

alter table public.ai_confirmations drop constraint if exists ai_confirmations_action_check;
alter table public.ai_confirmations add constraint ai_confirmations_action_check check(action in('create_promotion','update_promotion','pause_promotion','reactivate_promotion','duplicate_promotion'));
...
begin
  ...
  if mutation_action='update_promotion' and mutation_payload->>'benefit' is null and mutation_payload->>'startsAt' is null and mutation_payload->>'endsAt' is null then raise exception 'NO_CHANGES'; end if;
  ...
  values(target_conversation,auth.uid(),target_org,selected.venue_id,mutation_action,mutation_payload||jsonb_build_object('promotionId',selected.id,'originalTitle',selected.title)) returning id into confirmation_id;
...
declare confirmation public.ai_confirmations; payload jsonb; source public.promotions; source_rule public.promotion_rules; result_id uuid; new_rule_id uuid; benefit numeric; start_at timestamptz; end_at timestamptz;
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  ...
  if confirmation.action in('pause_promotion','reactivate_promotion') then
    ...
    if payload->>'benefit' is not null then
    ...
    insert into public.promotions(venue_id,event_id,title,description,terms,starts_at,ends_at,active,created_by,campaign_id,activation_id)
    values(source.venue_id,source.event_id,coalesce(nullif(trim(payload->>'title'),''),source.title||' · copia'),source.description,source.terms,start_at,end_at,true,auth.uid(),source.campaign_id,source.activation_id) returning id into result_id;
  ...
  update public.ai_conversations set status='completed',state=jsonb_build_object('promotionId',result_id,'action',confirmation.action),updated_at=now() where id=confirmation.conversation_id;
...
revoke all on function public.prepare_agent_promotion_mutation(uuid,uuid,text,jsonb),public.execute_confirmed_agent_promotion_mutation(uuid) from public,anon;
grant execute on function public.prepare_agent_promotion_mutation(uuid,uuid,text,jsonb),public.execute_confirmed_agent_promotion_mutation(uuid) to authenticated;
